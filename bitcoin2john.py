...
def check_wallet(wallet_file):

    filename = wallet_file
    if read_wallet(json_db, filename) == -1:
        return (1,"%s: this wallet is not encrypted\n" % filename)
        #return None
    ...
    if cry_method != 0:
        return (-1,"%s: this wallet uses unknown key derivation method\n" % filename)
    ...
    if len(cry_salt) == 16:
        ...
        return (-2,"%s: this wallet uses unsupported salt size\n" % filename)
    ...
    if len(json_db['mkey']['encrypted_key']) != expected_mkey_len:
        return (-3,"%s: this wallet uses unsupported master key size\n" % filename)

    cry_master = json_db['mkey']['encrypted_key'][-64:]  # last two AES blocks are enough

    return (2,"$bitcoin$%s$%s$%s$%s$%s$2$00$2$00\n" %
